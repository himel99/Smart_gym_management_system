...
if(isset($_POST['save_info']))
{
	...
	$result1 = mysqli_query($con, "UPDATE `user_details` SET `age`= $age,`height`=$height,`weight`=$weight,`batch_time`=$batch_time,`exercise`=$exercise,`payment_date`=now(),`payment_status`=$payment_status  WHERE user_id=$id");
				...
				<tr>
					...
					<td>
						<select name="payment_status" style="width:500px;">
							<option value="">Select</option>
							<option value="1" <?= $row2['payment_status'] == 1 ? 'selected' : '' ?> >Paid</option>
							<option value="0" <?= $row2['payment_status'] == 0 ? 'selected' : '' ?> >Unpaid</option>
